...
function SidebarCard({
  ...
  return (
    <div
      className={`right-sidebar-card rounded-[10px] bg-white p-5 shadow-[0_8px_30px_rgba(0,0,0,0.08)] ${className}`}
    >
      {children}
    </div>
  );
...
type BlogDetailSidebarPart = 'hero' | 'content' | 'all';
...
export default function BlogDetailSidebar({
  ...
  const showHero = part === 'hero' || part === 'all';
  const showContent = part === 'content' || part === 'all';

  return (
    <aside className={`space-y-6 ${className}`.trim()}>
      {showHero ? (
        <>
          <DailyLearningCard className="right-sidebar-card" />

          <SidebarCard>
            <GradientHeading>Daily Quiz</GradientHeading>
