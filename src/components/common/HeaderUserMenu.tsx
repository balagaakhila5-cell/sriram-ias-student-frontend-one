...
function ProfileIcon({
  ...
  const iconFill = isLight ? "#333333" : "white";
...
const HeaderUserMenu: React.FC<{ theme?: 'light' | 'dark' }> = ({
  theme = 'dark',
}) => {
  const isLight = theme === 'light';
  ...
  useEffect(() => {
    const markHydrated = () => useAuthStore.getState().setHydrated(true);
  ...
  return (
    <div className="relative z-[60]" ref={menuRef}>
      <button
        ...
        className={`flex h-[30px] w-[30px] cursor-pointer items-center justify-center rounded-full border-2 text-[13px] font-bold transition-colors ${
          isLight
            ? 'border-[#00679C] bg-[#00679C] text-white hover:bg-[#005580]'
            : 'border-white bg-white text-[#00679C] hover:bg-white/90'
        }`}
      >
        {initial}
      </button>
