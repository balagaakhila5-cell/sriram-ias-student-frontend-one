...
import type { HomepageCourse } from "@/features/homepage/types";

gsap.registerPlugin(ScrollTrigger);

const ExploreCourses: React.FC = () => {
  const { data: homepage, isLoading } = useHomepage();
  const sectionCourses = homepage?.sectionCourses;
  const homeCategories = sectionCourses?.categories ?? [];

  const tabs = useMemo(() => {
    return homeCategories.map((category) => category.name);
  }, [homeCategories]);
  ...
  const visibleCourses = useMemo(() => {
    ...
    const activeCategory =
      homeCategories.find((category) => category.name === activeTab) ??
      homeCategories[0];
    return (activeCategory?.courses ?? []) as HomepageCourse[];
  ...
  return (
    <section
      ref={sectionRef}
      className="relative py-20 px-4 md:px-8 lg:px-16 bg-white"
    >
      <div className="relative z-10 max-w-7xl mx-auto space-y-12">
        <div ref={headerRef} className="text-center">
          <h2 className="global-section-heading">
            {sectionCourses?.title ?? "EXPLORE OUR COURSES"}
          </h2>
        </div>

        <div
          ref={tabsRef}
          className="w-full bg-[#F5F5F5] rounded-full p-2 flex relative overflow-x-auto scrollbar-hide"
        >
          <div
            ref={indicatorRef}
          ...
          >
            {visibleCourses.length > 0 ? (
              visibleCourses.map((course) => {
                ...
                const image = course.bannerImage?.trim();
                const wrapperClassName = `group relative overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.06)] hover:shadow-[0_20px_40px_rgba(0,0,0,0.12)] transition-shadow duration-500 bg-white rounded-lg ${courseSlug ? "cursor-pointer" : "cursor-default"} w-full block`;
                ...
                return (
                  <div key={course._id} className={wrapperClassName}>
                    {cardBody}
                  </div>
                );
              })
